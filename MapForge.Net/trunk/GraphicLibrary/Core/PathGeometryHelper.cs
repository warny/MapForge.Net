...
namespace GraphicLibrary.Core
{
    ...
    {
        // Cache stacks
        Stack<PathGeometry> pathStack = new Stack<PathGeometry>();
        Stack<PathFigure> figStack = new Stack<PathFigure>();
        Stack<PolyLineSegment> segStack = new Stack<PolyLineSegment>();
        ...
        void FlattenGeometry(PathGeometry pathGeoDst, Geometry geoSrc, double tolerance, Matrix matxPrevious)
        {
       
#if SILVERLIGHT
            MatrixHelper matxHelper = new MatrixHelper(TransformHelper.GetTransformValue(geoSrc.Transform)) * new MatrixHelper(matxPrevious);
        ...
        void AddCorner(PolyLineSegment segDst, Matrix matx, Point pt1, Point pt2, 
        ...
        public void DumpFigureToList(List<Point> points, PathFigure figSrc)
        {
            ...
            foreach (PathSegment seg in figSrc.Segments)
            {
                ...
                {
                    for (int i = 0; i < (seg as PolyLineSegment).Points.Count; i++)
                        if (!pointsEqual((seg as PolyLineSegment).Points[i], points[points.Count - 1]))
                            points.Add((seg as PolyLineSegment).Points[i]);
                }
            }
